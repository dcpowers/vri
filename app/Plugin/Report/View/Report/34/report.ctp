    ...
    <div class="col-md-12">
        ...
        if(!isset($ext[2]) || $ext[2] != 'pdf'){
            ...
            <div class="pull-right hideLink">
                ...
                echo $this->Html->image("pdfPrint.gif", array(
                    ...
                    'width'=>'48px',
                    'height'=>'48px',
                    'url' => array('plugin'=> 'report', 'controller' => 'report', 'action' => 'smarts', $id ,'member'=>false, 'ext'=>'pdf'),
        ...
        <div class="widget widget-no-header">
            ...
            <div class="logo" style="text-align: center">
                <?php
                echo $this->Html->image("SMARTS.jpg", array(
                    'fullBase' => true,
                    'alt' => 'iWorkZone'
                ));                    
                ?>
            ...
            foreach($report as $item){
                ...
                <?php
                foreach($item['children'] as $key=>$section){
                    $text = str_replace('{uploadir}', $folder, $section['Report']['text']);
                    $text = str_replace('{key}', $section['Report']['id'], $text);
                    $text = str_replace('{firstname}', $userInfo['first_name'], $text);
                    ?>
                    <div class="col-md-12">
                    ...
                    foreach($section['children'] as $sectionKey=>$statement){
                        #pr($statement);
                        foreach($answers[34] as $key=>$answer){
                            if(isset($answer['section']) && $statement['Report']['parent_id'] == $answer['section']){
                                if($statement['Report']['min_score'] <= $answer['total'] && $statement['Report']['max_score'] >= $answer['total']){
                                    $text = str_replace('{firstname}', $userInfo['first_name'],$statement['Report']['text']);
                                    ?>
                                    <div class="col-md-12">
                                        <?=$text?>
                                    </div>
